...
cd("test_measurement_13_12_2022\"); % change depending on folder name
subject_name = 'SubjectK:'; % change depending on subject name
...
for trial_number = 1:number_trials
    ...
    for j = 1:(number_parameters -1)
        ...
        vicon.sorted.(strcat("trial_",num2str(trial_number))).(cell2mat(parameters_list(index_parameters(j),2))) = cell((size(current_vicon_data,1)) - row_parameters_data_start +3, (2+current_param_column_count));
        ...
        for k = 1:data_points
        vicon.sorted.(strcat("trial_",num2str(trial_number))).(cell2mat(parameters_list(index_parameters(j),2))){2+k,1} = cell2mat(current_vicon_data(row_parameters_data_start -1 + k, 1));
        vicon.sorted.(strcat("trial_",num2str(trial_number))).(cell2mat(parameters_list(index_parameters(j),2))){2+k,2} = cell2mat(current_vicon_data(row_parameters_data_start -1 + k, 2));
        for g = 1:size(temp_parameter_data,2)
        vicon.sorted.(strcat("trial_",num2str(trial_number))).(cell2mat(parameters_list(index_parameters(j),2))){2+k,2+g} = temp_parameter_data{k, g};
        end
        end
    end
end
